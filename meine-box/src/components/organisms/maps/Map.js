    ...
    async function initializeMap() {
      ...
      boxes.forEach((box, key) => {
        ...
        if (myBoxesId.some((e) => e.id === box.id && e.level === "low")) {
          ...
          popUps.push(
            `<h4 class="map-popup__title">SalesBox ${box.id} </h4><p class="map-popup__stock">Stock is low</p> <p class="map-popup__street">${box.street_address}</p> <a class="map-popup__link" href="/productsoverview/${box.id}">See products</a>`
          );
        } else if (
          myBoxesId.some((e) => e.id === box.id && e.level === "empty")
        ) {
          markerColors.push({ color: "#fa4359" });
          popUps.push(
            `<h4 class="map-popup__title">SalesBox ${box.id}</h4> <p class="map-popup__stock">Stock is empty</p> <p class="map-popup__street">${box.street_address}</p> <a class="map-popup__link" href="/productsoverview/${box.id}">See products</a>`
          );
        } else if (
          myBoxesId.some((e) => e.id === box.id && e.level === "full")
        ) {
          markerColors.push({ color: "#65b22e" });
          popUps.push(
            `<h4 class="map-popup__title">SalesBox ${box.id}</h4> <p class="map-popup__stock">Stock is good</p> <p class="map-popup__street">${box.street_address}</p> <a class="map-popup__link" href="/productsoverview/${box.id}">See products</a>`
          );
        } else {
          markerColors.push({ color: "#30C5FF" });
          popUps.push(
            `<h4 class="map-popup__title">SalesBox ${box.id}</h4> <p class="map-popup__street">${box.street_address}</p>`
          );
        }
      });
